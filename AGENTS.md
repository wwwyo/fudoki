...
- **⚠️「款コードは法定の款番号と一致する」は誤りだった。** 施行規則の別記を原文から起こした結果、
  法定の市町村歳出の款は 1〜14 で、款11 は災害復旧費である。災害復旧費を持たない狛江市（と三鷹市）は
  11 が公債費で、**以降のコードが法定から詰まってずれている**。当初の「裏づけ」（款数の一致・節や所属の整合）は
  款の中身を見ただけで、番号の対応は確認していなかった。名称の対応は正しかったがコードの主張が誤り。
  この誤りがマスタ（`account_master` + `account_map`）を作る直接の理由になった — コードは団体ごとに
  ずれるので、対応表なしに団体をまたいで「同じ款」を言えない
...
地方自治法施行規則 別記「歳入歳出予算の款項の区分及び目の区分」（第15条関係）の市町村・歳出の表を
原文から起こしてマスタにし（`bun run fetch:account-master` → `dbt/seeds/budget/account_master.csv`）、
...
- **法定に無い科目は誤りではない**（様式の備考1が行政権能の差による追加を認める）。
  `kind=addition` として明示登録する（例: 三鷹市のスポーツ推進費・生涯学習費、狛江市の幼児教育費）
- **目は揃えない。** 目以下の横断は COFOG が担う（科目の再編を同一視する根拠が無い）
- 適用範囲は**一般会計の歳出だけ**（様式の備考6: 特別会計は長が定めた区分による）
