...
| **Load** | 原典1行を1行のまま**標準の形**へ（①=FDP／②=OCDS／③=Popolo）。**親への参照を保持**（①=款・項・目・節／③=会議・議事日程・議案）。コードと名称の分離、単位の正規化、識別子の付与、列の意味づけ | **fudoki の判断を足すこと**（分類、名寄せ、推定）。階層を潰すこと |
| **Transform** | **判断が入る段**。COFOG 写像、事業者の名寄せ、話者の名寄せ、職名↔氏名の解決、表記揺れの吸収 | 取得 |

**段の切れ目は「fudoki の判断が入るかどうか」で引く。** ELT という語の一般的な意味（Load は生のまま置き、Transform で整形する）とはずれるが、この PJ で守りたい境界は整形の有無ではなく判断の有無である。
Load の出力（正本）は標準に適合していながら判断を含まないので、**原文と突き合わせて検証できる**。
Transform の出力（派生）は自治体が言っていないことを含むので、別のファイルとして配る。
