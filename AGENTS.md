...
🚧 **この節は移行先を書いている。現在の実装は TypeScript 単体で、ingestion も dbt もまだ無い**（「① 予算」節が現状）。移行の経緯と実測値は `.agent/design-doc-elt-restructure.md`。
...
- **取得（ingestion）**: Python。原典を Parquet で `data/raw/` へ落とす
