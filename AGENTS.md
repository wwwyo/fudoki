...
派生を CC BY 4.0 にできるのは原典が継承を求めないからで、CC BY-SA の団体が入れば
...
再配布可でない取得元（自治体サイトの PDF など）でも、そこから**抽出した事実**
（事業名・金額・コード）は配布物に入る。著作権法2条1項1号は著作物を「思想又は感情を
創作的に表現したもの」と定義しており、事業名と金額はそれにあたらないからである。
ここを混同すると、**CSV が無い約60団体の配布物が丸ごと空になる**。
成り立つのは事実だけで、事業説明のような散文を持ってくると表現の複製になる。

その帰結として **`raw/` の保証は取得元ごとに違う**。`sources.toml` の `raw_form` が宣言する。

| `raw_form` | 中身 | 検査 | 置ける条件 |
|---|---|---|---|
| `verbatim` | 原文そのもの | 復号の可逆性・原文の復元 | `redistribute = allow` のときだけ |
| `extracted` | 原文から抽出した事実 | **不可逆なので復元は検査できない** | 制限なし |

`extracted` の団体では「原典と1対1」が成立しないので、再現性の保証は
**証跡（URL・SHA-256・取得時刻）と抽出コードから取り直せること**に変わる。
...
core の出力（派生）は自治体が言っていないことを含むので、別のパッケージとして配る。
...
`derived/` という名前にカバレッジも中身も書かないのは、対象を広げたときや②③の派生が増えたときに嘘になるから
（中身は `datapackage.json` の `title` と `description` が言う）。
