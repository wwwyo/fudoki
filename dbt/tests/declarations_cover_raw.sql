...
--
-- ここは**原典の側**を母集団にして、宣言と突き合わせる。
-- ⚠️ 同じ型の検査は他に2本ある（`amount_declarations_cover_years` は金額の宣言が
-- 原典の全年度を覆っているか、`source_year_column_scope_is_real` は年度の列を宣言から
-- 外した年度に列が本当に無いか）。**この検査が見るのは (団体, direction) の存在だけ**で、
-- 宣言が年度で絞れるようになった以上、年度の抜けはそちらが見る。
...
{#
  ...
  下の glob に掛からない。**当時ここが原典の側を母集団にする唯一の検査で、そこに穴が空いていた** —
