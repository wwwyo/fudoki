{{ config(materialized = 'external', location = '../data/budget/datapackages/132071/revenue.csv', format = 'csv') }}
...
--
-- ⚠️ **他の3団体と列を揃えない。** 昭島市の歳入は説明欄が節の内訳（1段）で、
-- 歳出と階層の並びが違う。**歳入の節にはコードがある**（節の区分欄から取れる）。
-- 揃えることが「同じ概念だ」という判断になるので、正本は団体ごとの形のまま出す。
{% set amounts = var('budget_amounts')['132071']['revenue'] %}
{#- ⚠️ **宣言の件数で見る。** 止めたいのは段階が増えた場合（行の展開が要る）と、
    年度で宣言が割れた場合（列名・単位・倍率を年度で選ぶ形が要る）の両方で、
    このモデルはどちらにも対応していない。 -#}
{% if amounts | length != 1 %}
  {{ exceptions.raise_compiler_error(
      '132071/revenue: 金額の宣言が ' ~ amounts | length ~ ' 件ある。'
