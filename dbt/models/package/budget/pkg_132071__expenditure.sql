{{ config(materialized = 'external', location = '../data/budget/datapackages/132071/expenditure.csv', format = 'csv') }}
...
-- 目の下は説明欄の3段（事業 → 節 → 細節）である。**説明欄の節にはコードが無い**ので
-- `setsu_code` は空になる（コードを持つのは目の集計としての節列のほうで、名前で
-- 突き合わせるのは判断なので取り込みではやらない）。揃えることが「同じ概念だ」という
-- 判断になるので、正本は団体ごとの形のまま出す。
{% set amounts = var('budget_amounts')['132071']['expenditure'] %}
{#- ⚠️ **宣言の件数で見る。** 止めたいのは段階が増えた場合（行の展開が要る）と、
    年度で宣言が割れた場合（列名・単位・倍率を年度で選ぶ形が要る）の両方で、
    このモデルはどちらにも対応していない。 -#}
{% if amounts | length != 1 %}
  {{ exceptions.raise_compiler_error(
      '132071/expenditure: 金額の宣言が ' ~ amounts | length ~ ' 件ある。'
...
select
    ...
    source_amount
from {{ ref('stg_132071__expenditure') }}
