{{ config(materialized = 'external', location = '../data/budget/datapackages/131016/expenditure.csv', format = 'csv') }}
...
--
-- ⚠️ **他の団体と列を揃えない。** 千代田区の説明欄は事業 → 内訳の2段で、**節が行に付かない**。
-- 節は目の内訳として左頁の別の欄にあり、説明欄とは別の分解になっている（どちらも目の額に一致する）。
-- 名前で結び付けるのは名寄せ（判断）なので取り込みではやらない。揃えることが
-- 「同じ概念だ」という判断になるので、正本は団体ごとの形のまま出す。
{% set amounts = var('budget_amounts')['131016']['expenditure'] %}
{#- ⚠️ **宣言の件数で見る。** 止めたいのは段階が増えた場合（行の展開が要る）と、
    年度で宣言が割れた場合（列名・単位・倍率を年度で選ぶ形が要る）の両方で、
    このモデルはどちらにも対応していない。 -#}
{% if amounts | length != 1 %}
  {{ exceptions.raise_compiler_error(
      '131016/expenditure: 金額の宣言が ' ~ amounts | length ~ ' 件ある。'
...
select
    ...
    source_amount
from {{ ref('stg_131016__expenditure') }}
