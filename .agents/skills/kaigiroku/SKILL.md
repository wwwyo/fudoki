...
### 「画面を描画すれば取れる」は成立しない（検討して却下）

`Allow: /tenant/` の画面をブラウザ相当で描画して解析する案を検討したが、**却下した**。

- tenant ページは空テンプレートで、**本文は JS が `/dnp/search/*` から取る**。つまり描画すれば必ず Disallow 経路を叩く。実ブラウザを使ってもそのリクエストが Allow に変わるわけではない
- REP の適用主体は「自動化されたクライアント」。**実ブラウザでもスクリプトが動かせば crawler** であり、「実ブラウザである」ことと「人間が閲覧している」ことは別
- 負荷の面でも、静的アセットまで取るぶん API 直叩きより重い

**したがって DNP は許諾を取ってから API を直接使う。**回り込む方法を探さない。tenantId の実測も Disallow 経路への問い合わせなので、許諾前には行わない（manifest の DNP 15団体のうち2件で tenantId が空なのはこの理由）。

### User-Agent は連絡先入りの独自名を名乗る

```
kotonoha/0.1 (+https://github.com/wwwyo/kotonoha)
```

理由が3つある。

1. **`Mozilla/5.0` で通すのは robots を守る姿勢と矛盾する。**ブラウザを騙る時点で「許可されたものだけ取る」という主張が崩れる
2. **既存のブロックリストのどれにも該当しないので `User-agent: *` グループが適用される。**判定が単純になり、意図せず他人向けのルールを踏まない
3. **相手が後から `kotonoha` を名指しで拒否できる状態になる。**これが一番重要で、拒否の手段を相手に残しておくのが誠実

### UA でブロックされる自治体がある

CDN（CloudFront 等）の bot 対策で、**独自 UA も `curl` も 403 になり、ブラウザ UA だけ 200 が返る**サイトがある。実測（2026-08-10・東村山市）:

| UA | 結果 |
|---|---|
| `kotonoha/0.1 (+https://github.com/wwwyo/kotonoha)` | **403** |
| `curl/8.0` | 403 |
| ブラウザ UA | **200** |

対応は**実ブラウザで描画する**こと。ブラウザ UA を騙って HTTP を投げるのではなく、実際にブラウザを動かす。前者は騙りだが後者は事実。

⚠️ ただしこの場合、**相手からは人間の閲覧と区別がつかなくなる**。だからレート制御の責任が一層重い。同時実行1・取得済みスキップ・429/5xx で即停止を必ず守る。

### robots.txt の読み方で間違えやすい点

- **UA 指定のルールはその UA にしか適用されない。**`User-agent: CCBot/2.0` の `Disallow: /` は CCBot にしか効かない。自分に効くのは `User-agent: *` グループ（と自分の UA を名指ししたグループ）だけ
- **`User-agent: *` グループが存在しないファイルがある。**その場合、独自 UA には適用ルールが1件も無いことになる。ただし**書き手の意図としては全体に効かせるつもりだった可能性が高い**ので、技術的な抜け道として使わない（中央区がこの形）
- **先頭数行で判断しない。**26 UA 並ぶファイルもある。必ず全文を取って `User-agent:` を全部列挙してから判断する（実際にこれで VOICES 8団体を誤判定した）
