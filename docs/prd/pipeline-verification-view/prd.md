...
- **原典**: 自治体が公開した予算・決算の資料そのもの（CSV または PDF）
- **抽出**: PDF の原典から表を起こすこと。原典へ戻して突き合わせることはできない
- **復元検査**: 取り込んだデータから原典のファイルをそのまま戻せるかの検査。CSV の原典にだけ成り立つ
- **証跡**: 原典をいつ・どこから・どの版の手順で取り込み、どう確かめたかの記録
- **検査**: 配布物を書き出す前に通す自動のチェック
- **誤読の罠**: 見た目どおりに読むと誤った結論になる箇所。団体ごとに記録する
