...
4. **注意点が画面に出ない。** 団体ごとの注意点（千代田区なら「歳出の款が法定の体系とまったく違う」「人件費が職員費に集められている」など）は報告データに記録されているのに、画面には1件も表示されない。
...
この画面は `## Story` のどの Core action からも直接には導けない。運営者のための道具だからである。Core action「数字が怪しいと思ったときに、原典と合っているか・注意点が無いかを確かめる」は、利用者が repo（原典・証跡・配布物）を git で見て行う。
...
出所（資料名・取得元 URL・ページ範囲など）、取り込みの検証、検査は、それぞれが属する側のパネル内の詳細欄にまとめ、開閉できるようにする。検査はテスト名ではなく、何を確かめる検査かを説明する文言で出し、警告・失敗には詳細を添える。注意点は画面の上部に件数を出し、開くと一覧が見える。
...
- 警告・失敗・注意点という「見るべきもの」が、成功した検査の中に埋もれない
...
- **注意点（caveat）**: 見た目どおりに読むと誤った結論になる箇所。団体ごとに記録する
...
- [ ] 注意点が画面の上部に件数付きで出て、開くと団体ごとの一覧が見える
- [ ] 選んだ団体に関係する警告・失敗・証跡・注意点は、画面のどこかからすべて辿れる（見落としがない）
...
- コアアクション: パイプラインを回した後に `/pipeline/` を開き、組を選んで入力と出力・検査・注意点を見て、配布判断に必要な確認を終える
