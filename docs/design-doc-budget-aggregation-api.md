...
**訂正（decision.log #34）**: この実測は旧パッケージ名（v1 monolithic）だけを見た誤りだった。
typescript-sdk は v2 でパッケージ分割され、`@modelcontextprotocol/server` 2.0.0（2026-07-27 公開）が
現行版 2026-07-28 に既に対応していた。移行後は legacy（〜2025-11-25）と modern（2026-07-28）の両方を出す。
